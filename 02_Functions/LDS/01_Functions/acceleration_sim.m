...
%date:      18.12.19
%modified:  Korbinian Moller, TUM - 20.05.2021
%% Input: 
% characteristic
...
%Calculate e_i value
vehicle = calc_e_i_new(vehicle,Parameters.LDS);
...
if acc_time_is_stf > vehicle.LDS.settings.t_sim_max_acc || ~isnan(vehicle.LDS.sim_acc.acc_time_req)
    ...
    if ~isnan(vehicle.Input.T_max_Mot_r) || ~isnan(vehicle.Input.T_max_Mot_f) && abs(acc_time_is_stf - vehicle.LDS.sim_acc.acc_time_req) > max_diff_to_acc
        
        vehicle.LDS = errorlog(vehicle.LDS,'The acceleration time differs too much from the specified value. Motor torque will be scaled to achieve the required acceleration time!');
    ...
    while acc_delta > max_diff_to_acc || acc_delta < (-max_diff_to_acc) 
        ...
        % Traction limit is reached in at least 80% of the timesteps --> required acceleration time is to short, increase required time in input parameters
