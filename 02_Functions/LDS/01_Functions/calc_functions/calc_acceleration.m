...
for nm = find(filled_axles)
    ...
    n_motor(nm,:) = n_wheel.*vehicle.LDS.GEARBOX{nm}.i_gearbox(1);                                                                                 
        
    %interpolate values from the T_max motor curve
    F = griddedInterpolant(vehicle.LDS.MOTOR{nm}.characteristic(:,2),vehicle.LDS.MOTOR{nm}.characteristic(:,1),'linear','none');

    %Find torque of all motors on the axle in Nm
    T_motors(nm,:) = vehicle.LDS.MOTOR{nm}.quantity*F(n_motor(nm,:));                                                 

    %torque of wheels in Nm
    T_wheels(nm,:) = T_motors(nm,:).*vehicle.LDS.GEARBOX{nm}.i_gearbox(1).*vehicle.LDS.GEARBOX{nm}.eta(1);                                                                                                                                                     
...
vehicle.LDS.sim_acc.a_limited = acc_limited;
